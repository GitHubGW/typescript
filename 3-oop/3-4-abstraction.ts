{
  /**
   *  추상화: 외부에서 클래스를 사용할 때 어떻게, 무엇을 사용할 것인지를 고민하면서 클래스에서 내부 또는 외부에서 필요한 기능을 나누는 프로세스 자체를 추상화라고 부른다.
   *  클래스를 implements를 통해 여러 인터페이스를 구현하게 되면 해당 클래스는 인터페이스가 가지고 있는 프로퍼티나 함수들 또한 가지고 있어야 한다.
   */
